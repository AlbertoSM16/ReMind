...
El backend de la aplicación se ha estructurado siguiendo un patrón de arquitectura por capas, organizando el código fuente en paquetes independientes con responsabilidades bien definidas: \textit{entity}, \textit{mapper}, \textit{repository}, \textit{service}, \textit{controller}, \textit{security}, \textit{dto} y \textit{utils}.
...
El sistema de persistencia se compone de nueve entidades JPA que reflejan el modelo relacional de la base de datos. A continuación se describen las más relevantes:
...
\begin{itemize}
    ...
    \item \textbf{JuegoAgenda:} Tabla intermedia que asigna un juego a una agenda con un nivel de dificultad (\texttt{dificultad}, valor entero), una fecha de asignación y un campo \texttt{realizado} que indica si el paciente ha completado la actividad.
...
\begin{itemize}
    \item \textbf{DTOs de entrada:} \texttt{PacienteInputDTO}, \texttt{TerapeutaInputDTO}, \texttt{AgendaInputDTO}, \texttt{AdministradorInputDTO}, \texttt{AssignJuegoDTO} y \texttt{PacienteTerapeutaInputDTO}. Contienen anotaciones \texttt{@NotBlank}, \texttt{@NotNull} y \texttt{@Size} para validar los datos recibidos.
    \item \textbf{DTOs de salida:} \texttt{PacienteOutputDTO}, \texttt{TerapeutaOutputDTO}, \texttt{AgendaOutputDTO}, \texttt{AdministradorOutputDTO}, \texttt{JuegoAsignadoDTO}. Exponen únicamente los campos necesarios para cada vista.
    ...
    \item \textbf{DTOs de seguimiento:} \texttt{PacienteSeguimientoDTO} y \texttt{TerapeutaSeguimientoDTO}. Contienen datos agregados como el número de juegos completados y el total de juegos asignados, usados en el panel de monitorización del terapeuta.
    \item \textbf{DTOs auxiliares:} \texttt{PasswordResetDTO} (devuelve la nueva contraseña tras un reseteo) y \texttt{LoginRequest} (recibe usuario y contraseña en el login).
\end{itemize}

El mapeo entre entidades y DTOs se realiza mediante MapStruct. Para cada entidad principal existe una interfaz mapper anotada con \texttt{@Mapper(componentModel = "spring")} que define los métodos de conversión. Por ejemplo, \texttt{PacienteMapper} convierte \texttt{PacienteInputDTO} a \texttt{Paciente} y \texttt{Paciente} a \texttt{PacienteOutputDTO}, resolviendo automáticamente la relación con el terapeuta mediante \texttt{@Mapping(source = "terapeuta.id", target = "terapeuta\_id")}.
...
El frontend Angular se ejecuta en \texttt{http://localhost:4200}, por lo que la configuración global de CORS en Spring Boot restringe el origen permitido a esa dirección. Las anotaciones \texttt{@CrossOrigin(origins = "*")} en cada controlador quedan sobrescritas por esta configuración global.
...
\begin{itemize}
    ...
    \item \textbf{Generación de contraseña:} Construye una contraseña temporal con el formato \texttt{NOap\#\#!}, donde las dos primeras letras son las iniciales del nombre en mayúscula, las dos siguientes son las últimas letras del apellido en minúscula, seguidas de un número aleatorio entre 1 y 100 y un carácter especial.
\end{itemize}
